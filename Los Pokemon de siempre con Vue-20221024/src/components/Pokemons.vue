<template>
  <div>
    <h1>Pokemons a buscar</h1>
    <button @click="loadPokemons(15)">Cargar Pokemons</button>

    <p>
      Filtrar:
      <input type="text" v-model="search" placeholder="Buscar pokemon..." />
      &nbsp;
    </p>
    ...
    <table v-if="numberPokemons > 0">
      ...
      <tbody>
        <tr v-for="pokemon in filteredPokemons" :key="pokemon.url">
    ...
    <div>
      ...
      <p>Aunque tú habías pedido {{ cantidad }}</p>
    </div>
    ...
    <div v-if="!!pokemonSelected && numberPokemons > 0">
      ...
      <img :src="pokemonSelected.sprites.front_default" alt="imagen" />
    </div>
  </div>
</template>
